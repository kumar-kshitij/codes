...
void dfs(vl graph[], vl &tour, ll first[], ll height[], ll u, ll h) {
    first[u] = tour.size();
    tour.pb(u);
    height[u] = h;
    for (ll v : graph[u]) {
        dfs(graph, tour, first, height, v, h + 1);
        tour.pb(u);
    }
}
...
int main() {
    ...
    while (t--) {
        ...
        height[0] = INT_MAX;
        dfs(graph, tour, first, height, 1, 0);
        vl tree;
        ...
        while (q--) {
            ...
            cout << query(tree, height, 1, 0, tour.size() - 1, a, b) << nl;
        }
    }
    return 0;
}
